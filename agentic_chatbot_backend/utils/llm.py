...
client = genai.Client(api_key=Config().GOOGLE_GENERATIVE_AI_API_KEY)

def read_system_instruction(file_path: str = "../prompt/Chat_Bot_prompt.txt") -> str:
    file_path = os.path.join(os.path.dirname(__file__), "../prompt/Chat_Bot_prompt.txt")
    try:
...
class LlmGenerator:
    def __init__(
        ...
        self.client = genai.Client(api_key=Config().GOOGLE_GENERATIVE_AI_API_KEY)
